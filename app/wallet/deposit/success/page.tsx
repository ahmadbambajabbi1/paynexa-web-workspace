...
function DepositSuccessRedirect() {
  ...
  useEffect(() => {
    ...
    if (source === "app") {
      setAppHandoff(true);
      const deepLink = buildAppDepositDeepLink("success", context, id);
    ...
    const target =
      raw && raw.startsWith("/") && !raw.startsWith("//")
        ? withDepositState(raw, "success")
        : withDepositState("/dashboard", "success");
    ...
    <div className="flex min-h-screen items-center justify-center bg-app-canvas px-4">
      <div className="text-center">
        ...
        <p className="mt-4 text-sm font-medium text-gray-600">
          {appHandoff ? `Opening ${APP_NAME} app…` : `Payment received. Returning you to ${APP_NAME}…`}
        </p>
      </div>
    </div>
  );
